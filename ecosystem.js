/* ecosystem.js v1.0.0 — связка сайтов Betaline.
 ...
 *
 * Элемент 1 — оранжевая полоска продуктов (все сайты). Прячется при прокрутке вниз,
 *   возвращается при остановке, прокрутке вверх, мыши у верхнего края, вверху страницы.
...
(function () {
  ...
  var H = 36, TOP_ZONE = 80, BACK_MIN = 200, IDLE = 700;
  ...
  var css =
    ...
    'overflow-x:auto;white-space:nowrap;scrollbar-width:none;transition:transform .4s ease}' +
    '.eco::-webkit-scrollbar{display:none}' +
    '.eco.off{transform:translateY(-100%)}' +
    ...
    'background:#fff;border:1.5px solid #e8541e;color:#e8541e;padding:8px 14px;border-radius:999px;font:600 14px/1 Golos Text,-apple-system,Segoe UI,sans-serif;' +
    'text-decoration:none;box-shadow:0 4px 14px rgba(232,84,30,.18);opacity:0;transform:translateY(-8px);pointer-events:none;transition:opacity .3s,transform .3s}' +
    '.eco-back.on{opacity:1;transform:none;pointer-events:auto}' +
    '.eco-back:focus-visible{outline:2px solid #e8541e;outline-offset:2px}' +
    '.eco-back .m{display:none}' +
    '@media(max-width:719px){' +
    '.eco{gap:16px;padding:0 14px;font-size:12px}.eco b{display:none}' +
    '.eco-back{top:auto;left:12px;bottom:calc(var(--eco-back-bottom,12px) + env(safe-area-inset-bottom));white-space:nowrap;padding:10px 14px;transform:translateY(8px)}' +
    '.eco-back .d{display:none}.eco-back .m{display:inline}}' +
    '@media(prefers-reduced-motion:reduce){.eco,.eco-back{transition:none}}';
  ...
  // ---- поведение: один passive scroll + rAF + общий idle-таймер
  var lastY = window.pageYOffset, ticking = false, idleT = 0, mouseTop = false;
  function focusInside(el) { return el && el.contains(document.activeElement); }
  function apply(scrolling) {
    var y = window.pageYOffset, down = y > lastY + 2, up = y < lastY - 2;
    if (scrolling) { if (down || up) lastY = y; }
    var showBar = reduce || y < TOP_ZONE || mouseTop || focusInside(bar) || !scrolling || up;
    if (scrolling && down && y >= TOP_ZONE && !mouseTop && !focusInside(bar) && !reduce) showBar = false;
    bar.classList.toggle('off', !showBar);
    if (back) {
      var showBack = y >= BACK_MIN && (reduce || !scrolling || focusInside(back));
      back.classList.toggle('on', showBack);
    }
  }
  function onScroll() {
  ...
  if (fine) {
    window.addEventListener('mousemove', function (e) {
      var t = e.clientY < 40;
      if (t !== mouseTop) { mouseTop = t; if (t) apply(false); }
    }, { passive: true });
  }
  apply(false);
})();
