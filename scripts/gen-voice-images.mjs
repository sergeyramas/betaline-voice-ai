...
const CARTOON = `Flat line-art cartoon illustration in the exact style of a modern SaaS landing: clean vector outlines, warm palette with vivid orange (#F97316) accents, light cream-white background, one expressive character, subtle drop shadow under objects, no text and no logos anywhere, square composition. `;

const PROMPTS = {
  'problem-raw-bot': CARTOON + `An annoyed small-business owner in a blazer holds a smartphone at arm's length; out of the phone comes a clunky cardboard-looking toy robot head speaking with jagged broken speech-bubble (glitch zigzags inside), storm cloud and lightning above the owner's head. Mood: they sold me a dumb raw robot.`,
  'problem-burned-base': CARTOON + `A large smartphone standing upright covered in red prohibition/spam stickers and a big red exclamation warning shield; around it three small contact cards with unhappy faces sliding into a trash bin; small flames licking the bottom edge of the phone. Mood: the contact base got burned by spam calls.`,
  'problem-hangup': CARTOON + `An irritated customer in casual clothes slamming a phone receiver down / tossing the phone away, with angry motion lines; from the phone a tiny robot voice bubble is cut in half by a big hand gesture 'stop'. Mood: people instantly hang up on robots.`,
  'plan-network': `Premium soft 3D render in warm brand style: an isometric network map on a white rounded panel — orange and white glossy nodes connected by glowing orange pipes and thin lines, in the center a phone-call node with a waveform, branching to a document node, a shield node, a person avatar node and a CRM table node; light warm cream background (#FFF7ED), soft studio light, subtle warm glow, no text and no letters anywhere. Square composition, generous whitespace.`,
  ...
  'step-2-demo': STYLE + `A voice-scenario preparation card: a white script document card with greeked text lines, an orange bookmark and a small approval checkmark; beside it an orange audio waveform strip with a round play button and a small microphone icon; a subtle branching diagram of the dialogue (three greeked nodes connected by thin orange lines) beneath. Mood: script written, voice prepared, demo ready.`,
  'step-3-pilot': STYLE + `A controlled-pilot results card: a neat stack of three small call-record cards each with a phone icon and a status dot (green, orange, grey); beside them a compact report card with a simple orange bar chart and greeked caption lines; a small green checkmark pill at the corner. Mood: limited pilot finished, clear measurable outcome.`,
  'step-1-razbor': STYLE + `A call-task audit card: a white checklist card with greeked text lines where two rows have small green checkmarks and one row has an orange magnifying-glass icon; beside it a contact-base card showing three rows of tiny person avatars with greeked bars, topped by a soft orange shield icon with a checkmark. Mood: task and contact base carefully reviewed before launch.`,
};
