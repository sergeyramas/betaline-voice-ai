...
- [2026-07-29] **claude-mac-sonnet5** (mac, session-24079abd, чат «финальная визуальная правка гарантий») — topic: `guar-img-trim-crop` — branch: `main`@a52d9fa — DONE
  Корень мелкой картинки в #safety (см. предыдущую запись `guar-img-resize`) оказался не в max-width, а в самом файле: `guarantees-shield.png` был 1024×571, но фигура девушки со щитом внутри занимала только ~415×555 px — остальное прозрачные поля. Любой max-width тюнинг двигал невидимый бокс, а не видимую фигуру.
  Правка: обрезал прозрачные поля Pillow'ом (`im.convert('RGBA').getbbox()` → `crop`, порог alpha=0 давал стабильный bbox `(302,16,717,571)` на всех проверенных порогах 0/10/50/128), пересжал `pngquant --quality=70-90` (без trim+RGBA-передискретизации файл раздулся до 243КБ; после pngquant — 90КБ, даже меньше оригинальных 103КБ). Новый intrinsic-размер 415×555 — обновил `width`/`height` в `<img class="zv-guar-img">` (`index.html`) с 1024×571. `.zv-guar-img` max-width: десктоп 420→440px (визуальная фигура теперь = размеру элемента, попадает в целевые 420–460px на 1440), мобильный 260→220px (высота 220×(555/415)≈294px = 36% от 812px высоты экрана, под лимитом 40%). ImageMagick не установлен — весь пайплайн через `python3`+Pillow+`pngquant` (оба уже стояли).
  Верификация (Playwright, `.vercel/node_modules/@playwright/test` 1.61.1): локально и повторно на живом проде — 1440×900 и 375×812, light+dark (`data-theme=dark`). Все 4 комбинации: `naturalWidth/Height` = 415×555 (совпадает с новыми атрибутами, без layout shift), рендер-ширина картинки 440px (десктоп) / 220px (мобайл), `horizontalOverflowPx: 0`, `consoleErrors: []`. Скриншоты секции: `guar_v3_{desktop,mobile}_{light,dark}.png` в scratchpad сессии.
  Деплой руками (Actions по-прежнему сломан — невалидный `VERCEL_TOKEN`): `vercel pull/build/deploy --prebuilt --prod --scope sergeyramas-projects` → `dpl_123qr4LVBfdAFJWBSSD7SbvtMwRF`, алиас на `zvonok.betaline-ai.ru` сработал. Прод: `curl` 200 OK на `/` и на картинке, `styles.css` отдаёт новые `max-width: 440px`/`220px`, картинка на проде весит 90463 байт (совпадает с локальной), Playwright-смоук на живом URL — тот же результат (0 errors, 0 overflow, 440/220px).
  Не трогал: текст, аккордеоны, разметку секции кроме width/height атрибута картинки, другие секции страницы.
